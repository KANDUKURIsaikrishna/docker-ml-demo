# Live Demo Guide — Kubernetes (kind)

For presenting the Kubernetes half live. Assumes students have already seen the Docker Compose demo ([DEMO.md](DEMO.md)) — this doc focuses on what's *new*: pods instead of containers, service discovery, and self-healing. It doesn't re-cover the smile/glasses/eyes talking points — those are identical, see DEMO.md.

Every command below was run end-to-end against a real kind cluster before this doc was written (see commit history) — not just typed and hoped for.
...
Confirm `kind` and `kubectl` show OK. Build the images if you haven't already:
...
```bash
for svc in smile-service glasses-service eyes-service gateway frontend; do
  kubectl set image deployment/$svc $svc=ml-$svc:latest -n ml-demo
  kubectl patch deployment $svc -n ml-demo -p "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"$svc\",\"imagePullPolicy\":\"Never\"}]}}}}"
done
```
